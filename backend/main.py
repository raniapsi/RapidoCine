...
app = FastAPI(title="RapidoCine", description="IMDB Clone")

# Mount static files
app.mount("/static", StaticFiles(directory="static"), name="static")
templates = Jinja2Templates(directory="templates")

# Mock data with proper TMDB image URLs
movies_data = [
    {
        "id": 1,
...
async def home(request: Request):
    ...
    return templates.TemplateResponse("index.html", {
        ...
        "featured_movies": movies_data[:4]  # For carousel
    })
...
async def movie_detail(request: Request, movie_id: int):
    ...
    # Get similar movies (exclude current movie)
...
async def movie_list(request: Request, type: str):
    """Movie list page by type (popular, top_rated, wishlist)"""
    if type == "top_rated":
        filtered_movies = [m for m in movies_data if m["vote_average"] >= 8.5]
        title = "Top Rated"
    elif type == "wishlist":
        # Mock upcoming movies
        filtered_movies = movies_data[-2:].copy()  # Last 2 as upcoming
...
# Add this to make sure FastAPI can find the app
